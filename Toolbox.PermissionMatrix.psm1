...
function Test-ExpandedMatrixHC {
    ...
    .PARAMETER ADObjects
        ...
        details about each object. As generated by 'Get-ADObjectDetailHC'.
    #>
    [CmdletBinding()]
    [OutputType([PSCustomObject[]])]
    param (
    ...
    try {
        $AdNamesHash = [System.Collections.Generic.HashSet[string]]::new([StringComparer]::OrdinalIgnoreCase)
        foreach ($Obj in $ADObject) {
            $null = $AdNamesHash.Add($Obj.samAccountName)
        }

        $MatrixKeysHash = [System.Collections.Generic.HashSet[string]]::new([StringComparer]::OrdinalIgnoreCase)
        foreach ($Row in $Matrix) {
            if ($Row.ACL) {
                foreach ($Key in $Row.ACL.Keys) {
                    $null = $MatrixKeysHash.Add($Key)
                }
            }
        }

        #region Check if the matrix contains objects not available in ADObjects
        foreach ($Key in $MatrixKeysHash) {
            if (-not $AdNamesHash.Contains($Key)) {
                throw "Unknown AD Object '$Key' found in the matrix."
            }
        }
        #endregion

        #region Non existing AD Objects (Objects we queried but AD couldn't find)
        $ADObjectsUnknown = [System.Collections.Generic.List[string]]::new()
        foreach ($Obj in $ADObject) {
            if (-not $Obj.adObject -and $MatrixKeysHash.Contains($Obj.samAccountName)) {
                $ADObjectsUnknown.Add($Obj.samAccountName)
            }
        }

        if ($ADObjectsUnknown.Count -gt 0) {
            [PSCustomObject]@{
                Type        = 'FatalError'
                ...
                Value       = $ADObjectsUnknown.ToArray()
            }
        }
        #endregion

        #region Empty AD groups
        $EmptyAdGroups = [System.Collections.Generic.List[string]]::new()
        
        foreach ($Group in $ADObject.Where({ $_.adObject.ObjectClass -eq 'group' })) {
            $HasValidUser = $false
            
            # Check members until we find ONE valid user, then break early
            foreach ($Member in $Group.adGroupMember.SamAccountName) {
                if ($Member -notin $ExcludedSamAccountName) {
                    $HasValidUser = $true
                    break 
                }
            }
            ...
            if (-not $HasValidUser) {
                $EmptyAdGroups.Add($Group.samAccountName)
            }
        }

        # Filter empty groups to only those actually used in the matrix
        $UsedEmptyGroups = $EmptyAdGroups.Where({ $MatrixKeysHash.Contains($_) })
        
        if ($UsedEmptyGroups.Count -gt 0) {
            [PSCustomObject]@{
                Type        = 'Information'
                Name        = 'Empty groups'
                ...
                Value       = $UsedEmptyGroups
            }
        }
        #endregion

        #region Inaccessible folders
        # A valid object is a user, or a group that is NOT in the EmptyAdGroups list
        $ValidAdObjectsHash = [System.Collections.Generic.HashSet[string]]::new([StringComparer]::OrdinalIgnoreCase)
        foreach ($Obj in $ADObject) {
            if ($Obj.ADObject.ObjectClass -eq 'user' -or 
                ($Obj.ADObject.ObjectClass -eq 'group' -and $Obj.samAccountName -notin $EmptyAdGroups)) {
                $null = $ValidAdObjectsHash.Add($Obj.samAccountName)
            }
        }

        $InaccessiblePaths = [System.Collections.Generic.List[string]]::new()
        foreach ($Row in $Matrix) {
            if ($Row.ACL.Keys.Count -gt 0) {
                $FolderHasAccess = $false
                foreach ($Key in $Row.ACL.Keys) {
                    if ($ValidAdObjectsHash.Contains($Key)) {
                        $FolderHasAccess = $true
                        break
                    }
                }

                if (-not $FolderHasAccess) {
                    $InaccessiblePaths.Add($Row.Path)
                }
            }
        }
        ...
        if ($InaccessiblePaths.Count -gt 0) {
            [PSCustomObject]@{
                ...
                Value       = $InaccessiblePaths.ToArray()
            }
        }
        #endregion

        #region Duplicate AD objects between matrix and default
        if ($DefaultAcl.Count -gt 0 -and $MatrixKeysHash.Count -gt 0) {
            $DuplicateAdObjects = [System.Collections.Generic.List[string]]::new()
            
            # Simply check if the MatrixKeysHash contains any keys from DefaultAcl
            foreach ($Key in $DefaultAcl.Keys) {
                if ($MatrixKeysHash.Contains($Key)) {
                    $DuplicateAdObjects.Add($Key)
                }
            }

            if ($DuplicateAdObjects.Count -gt 0) {
                [PSCustomObject]@{
                    Type        = 'Information'
                    ...
                    Value       = $DuplicateAdObjects.ToArray() | Sort-Object
                }
            }
        }
        #endregion
